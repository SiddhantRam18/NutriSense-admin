import requests

Not_found_link = 'data:image/png;base64,'

_HEADERS = {'User-Agent': 'NutriSense/1.0'}


def get_images_links(searchTerm):
    clean_term = searchTerm.split('(')[0].strip()

    # TheMealDB — exact recipe images for well-known dishes, no key required
    try:
        resp = requests.get(
            'https://www.themealdb.com/api/json/v1/1/search.php',
            params={'s': clean_term},
            headers=_HEADERS,
            timeout=5,
        )
        data = resp.json()
        if data.get('meals'):
            return data['meals'][0]['strMealThumb']
    except Exception:
        pass

    # Openverse — open-licensed food images (Flickr/Wikimedia), no key required
    # Try progressively shorter queries if the full name has no results
    words = clean_term.split()
    queries = [clean_term] + ([' '.join(words[:2])] if len(words) > 2 else []) + ([words[0] + ' food'] if words else [])
    for query in queries:
        try:
            resp = requests.get(
                'https://api.openverse.org/v1/images/',
                params={'q': query, 'page_size': 1},
                headers=_HEADERS,
                timeout=8,
            )
            data = resp.json()
            if data.get('results'):
                return data['results'][0]['url']
        except Exception:
            pass

    return Not_found_link
